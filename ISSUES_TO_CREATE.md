...
This document contains detailed issue descriptions for the findings in the forensic audit. Each issue is ready to be copy-pasted into GitHub Issues.

---

## Issue #1: [CRITICAL] Refactor JpegParser - Extract Marker Handler Strategy Pattern

**Labels**: `refactoring`, `architecture`, `priority:high`, `SOLID:SRP`, `SOLID:OCP`
...
## Issue #2: [CRITICAL] Refactor IsoBmffParser - Introduce Parse Context Object (DRY)

**Labels**: `refactoring`, `DRY`, `priority:high`, `code-quality`
...
## Issue #3: [CRITICAL] Refactor ParsedExif - Extract Domain Adapters (SRP, SoC)

**Labels**: `refactoring`, `architecture`, `priority:high`, `SOLID:SRP`, `SoC`

**Description**:

### Problem
`src/Exif/Model/ParsedExif.php` is a mega god class (500+ LOC, 224 public methods) violating both Single Responsibility Principle and Separation of Concerns.
...
### Example
```php
class CameraMetadataAdapter
{
    public function __construct(private ParsedExif $exif) {}
    
    public function getMake(): ?string {
        // Extracted from ParsedExif::cameraMake()
    }
}

class ParsedExif
{
    // Core IFD data structure remains
...
## Issue #4: [HIGH] Eliminate Code Duplication in MakerNotes Decoders (DRY)

**Labels**: `refactoring`, `DRY`, `priority:medium`, `easy-fix`
...
## Issue #5: [HIGH] Extract Nested Logic in XmpParser (KISS)

**Labels**: `refactoring`, `KISS`, `priority:medium`, `code-quality`
...
## Issue #6: [HIGH] Introduce Dependency Injection for Parsers (SOLID:DIP)

**Labels**: `refactoring`, `architecture`, `SOLID:DIP`, `priority:medium`, `testability`
...
3. `src/MetadataReader.php` (L54-59):
```php
public function __construct(
    private TiffExifParser $tiffExifParser = new TiffExifParser(),
    private AppleMakerNotesMerger $appleMakerNotesMerger = new AppleMakerNotesMerger(),
...
4. `src/MetadataReader.php` (L112, 210):
```php
...
class IccParser implements IccParserInterface {
...
## Issue #7: [MEDIUM] Extract GpsConverter Encoding Decoders (DRY)

**Labels**: `refactoring`, `DRY`, `priority:low`, `code-quality`
...
// Usage
private function decodeUndefinedUtf8(string $payload): ?string {
    return $this->decodeUndefinedWithEncoding($payload, 'UTF-8');
}
```
...
## Issue #8: [MEDIUM] Add Configuration Objects for Parser Limits (CoC)

**Labels**: `enhancement`, `CoC`, `priority:low`, `configuration`
...
### Example
```php
readonly class JpegParserConfig
{
    public function __construct(
...
class JpegParser
{
    ...
    private function validateSegmentSize(int $size): void {
        if ($size > $this->config->maxAppSegmentSize) {
            throw new ParseError('Segment too large');
        }
    }
}
...
$parser = new JpegParser(
    new JpegParserConfig(maxAppSegmentSize: 8_388_608) // 8 MiB
);
```
...
## Issue #9: [MEDIUM] Replace instanceof Chains with Polymorphism (GRASP)

**Labels**: `refactoring`, `GRASP:Polymorphism`, `priority:medium`, `architecture`
...
{
    public function resolveValue(KeyedArchiveUnarchiver $unarchiver): mixed {
        // Dictionary-specific resolution logic
    }
}
...
## Issue #10: [LOW] Evaluate Factory Necessity (YAGNI)

**Labels**: `refactoring`, `YAGNI`, `priority:low`, `architecture-review`
...
## Issue #11: [DOCUMENTATION] Create Migration Guide for Refactored APIs

**Labels**: `documentation`, `priority:medium`, `user-facing`
...
- 🟠 High: 4 (Issues #4, #5, #6, #9)
- 🟡 Medium: 3 (Issues #7, #8, #10)
- 📝 Documentation: 1 (Issue #11)

**Estimated Total Effort**: 25-35 development days

**Recommended Order**:
1. Issue #4 (quick win, 1 day)
2. Issue #2 (medium complexity, high impact, 2-3 days)
3. Issue #1 (critical, high complexity, 3-5 days)
4. Issue #5 (medium complexity, 2 days)
5. Issue #6 (architectural, 3-4 days)
6. Issue #3 (critical but high-risk, 5-7 days)
7. Issue #7 (quick cleanup, 1 day)
8. Issue #9 (polymorphism, 3-4 days)
9. Issue #8 (configuration, 2-3 days)
10. Issue #10 (evaluation, 2-3 days)
11. Issue #11 (documentation, ongoing)

---

**Instructions for creating GitHub Issues:**

1. Copy each issue section (from "Issue #X" to the next issue)
2. Create new GitHub issue with title (e.g., "[CRITICAL] Refactor JpegParser - Extract Marker Handler Strategy Pattern")
3. Paste description
4. Add labels as specified
5. Set milestone if applicable
6. Assign to appropriate developer

**Note**: These issues reference `FORENSIC_AUDIT.md` which should be committed to the repository before creating issues.
