...
final readonly class GpsResolver
{
    ...
    public function resolve(?ExifDocument $exifDocument, ?XmpDocument $xmpDocument): ?Gps
    {
        ...
        $xmpLatRef = $this->uppercase(XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSLatitudeRef'));
        if ($latitudeRef === null) {
        ...
        if ($latitude === null) {
            $latitude = $this->parseCoordinate(
                XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSLatitude'),
                $xmpLatRef ?? $latitudeRef,
            );
        }

        $xmpLonRef = $this->uppercase(XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSLongitudeRef'));
        ...
        if ($longitude === null) {
            $longitude = $this->parseCoordinate(
                XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSLongitude'),
                $xmpLonRef ?? $longitudeRef,
            );
        }

        if ($altitude === null) {
            $altitudeXmp = XmpReader::float($xmpDocument, self::NS_EXIF, 'GPSAltitude');
            if ($altitudeXmp !== null) {
                $altRefXmp = $this->intValue(XmpReader::int($xmpDocument, self::NS_EXIF, 'GPSAltitudeRef'));
        ...
        $xmpSpeedRef = XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSSpeedRef');
        if ($speedRef === null) {
        ...
        $speedValue = XmpReader::float($xmpDocument, self::NS_EXIF, 'GPSSpeed');
        if ($speedValue !== null) {
        ...
        $xmpDestDistRef = XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSDestDistanceRef');
        if ($destDistRef === null) {
        ...
        $destDistValue = XmpReader::float($xmpDocument, self::NS_EXIF, 'GPSDestDistance');
        ...
        if ($date === null) {
            $date = $this->normaliseDate(XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSDateStamp'));
        }

        if ($time === null) {
            $time = $this->stringValue(XmpReader::string($xmpDocument, self::NS_EXIF, 'GPSTimeStamp'));
        }
    ...
    private function parseCoordinate(?string $value, ?string $ref): ?float
    {
        ...
        if (count($parts) === 3) {
            $deg = XmpReader::parseNumericString($parts[0]);
            $min = XmpReader::parseNumericString($parts[1]);
            $sec = XmpReader::parseNumericString($parts[2]);
        ...
        $numeric = XmpReader::parseNumericString($parts[0]);
        if ($numeric === null) {
            return null;
        }
    ...
    {
        $value = XmpReader::string($document, self::NS_EXIF, 'GPSDateTime');
        if ($value === null) {
            return null;
        }
