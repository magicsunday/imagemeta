...
final readonly class ParsedExif
{
    ...
    /**
     * Identifies the kind/type of the given IFD instance.
     *
     * EXIF 3.0 §4.6.3 and §4.5.2 define the major IFD categories (IFD0, Exif IFD, GPS IFD,
     * Interoperability IFD, and IFD1) used to organize EXIF metadata within TIFF structures.
     *
     * @param Ifd $ifd The IFD instance to identify.
     *
     * @return \MagicSunday\ImageMeta\Value\Enum\IfdKind|null Returns the IFD kind if identifiable, null otherwise.
     */
    public function identifyIfdKind(Ifd $ifd): ?\MagicSunday\ImageMeta\Value\Enum\IfdKind
    {
        // Check if it's IFD0 (root directory)
        if (spl_object_id($ifd) === spl_object_id($this->ifd0)) {
            return \MagicSunday\ImageMeta\Value\Enum\IfdKind::IFD0;
        }

        // Check if it's the Exif IFD
        if ($this->exifIfd instanceof Ifd && spl_object_id($ifd) === spl_object_id($this->exifIfd)) {
            return \MagicSunday\ImageMeta\Value\Enum\IfdKind::ExifIFD;
        }

        // Check if it's the GPS IFD
        if ($this->gpsIfd instanceof Ifd && spl_object_id($ifd) === spl_object_id($this->gpsIfd)) {
            return \MagicSunday\ImageMeta\Value\Enum\IfdKind::GPSIFD;
        }

        // Check if it's the Interoperability IFD
        if ($this->interopIfd instanceof Ifd && spl_object_id($ifd) === spl_object_id($this->interopIfd)) {
            return \MagicSunday\ImageMeta\Value\Enum\IfdKind::InteropIFD;
        }

        // Check if it's IFD1 (thumbnail/secondary image)
        if ($this->ifd1 instanceof Ifd && spl_object_id($ifd) === spl_object_id($this->ifd1)) {
            return \MagicSunday\ImageMeta\Value\Enum\IfdKind::IFD1;
        }

        // Check if it's a SubIFD
        foreach ($this->subIfds as $subIfd) {
            if (spl_object_id($ifd) === spl_object_id($subIfd)) {
                return \MagicSunday\ImageMeta\Value\Enum\IfdKind::SubIFD;
            }
        }

        // Could be MakerNotes or unknown
        return null;
    }
}
