...
final readonly class ValueFactory
{
    ...
    private function createFlashPix(array $streams): FlashPix
    {
        if ($this->flashPixParser instanceof FlashPixParserInterface) {
            $document = $this->flashPixParser->parse($streams);

            return $this->flashPixDocumentToValue($document);
        }
    ...
    /**
     * Converts a FlashPix document model to its value representation.
     */
    private function flashPixDocumentToValue(FlashPixDocument $document): FlashPix
    {
        $summaryInfo = null;

        if ($document->summaryData instanceof FlashPixSummaryData) {
            $summaryInfo = new FlashPixSummaryInfo(
                title: $document->summaryData->title,
                subject: $document->summaryData->subject,
                author: $document->summaryData->author,
                keywords: $document->summaryData->keywords,
                comments: $document->summaryData->comments,
                lastSavedBy: $document->summaryData->lastSavedBy,
                application: $document->summaryData->application,
                createTime: $document->summaryData->createTime,
                saveTime: $document->summaryData->saveTime,
            );
        }

        return new FlashPix($document->streams, $summaryInfo);
    }
}
