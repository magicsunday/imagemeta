...
final class AppleJpegIfdParserTest extends TestCase
{
    ...
    private function buildPayload(bool $bigEndian, array $entries, string $extraData = ''): string
    {
        ...
        $version   = "\x00\x01";
        $bo        = $bigEndian ? 'MM' : 'II';
        $magic     = $bigEndian ? pack('n', 0x002A) : pack('v', 0x002A);

        // IFD starts right after the 8-byte TIFF header (offset 8 from byte order)
        $ifdOffset = $bigEndian ? pack('N', 0x0008) : pack('V', 0x0008);
        ...
        return $signature . $version . $bo . $magic . $ifdOffset
    ...
    public function parseReturnsNullForInvalidByteOrder(): void
    {
        ...
        $raw = "Apple iOS\x00" . "\x00\x01" . "XX" . pack('n', 0x002A) . pack('N', 0x0008)
            . pack('n', 0) . pack('N', 0);
    ...
    #[Test]
    public function parseReturnsNullForBadTiffMagic(): void
    {
        $parser = new AppleJpegIfdParser();

        // Valid signature + version + byte order but wrong magic (99 instead of 42)
        $raw = "Apple iOS\x00" . "\x00\x01" . "MM" . pack('n', 99) . pack('N', 0x0008)
            . pack('n', 0) . pack('N', 0);
    ...
    public function parseSRationalAsFloat(): void
    {
        ...
        // SRATIONAL is 8 bytes → must be out-of-line
        // Extra data starts after: signature(10) + version(2) + TIFF header(8) + entry count(2) + 1 entry(12) + next IFD(4) = 38
        // Offset from TIFF base (offset 12 in blob): 38 - 12 = 26
        $extraData = $this->sRational(true, 3, 2);

        $payload = $this->buildPayload(true, [
            ['tag' => 0x000F, 'type' => self::TIFF_TYPE_SRATIONAL, 'count' => 1, 'value' => $this->offsetField(true, 26)],
        ], $extraData);
    ...
    public function parseSRationalListForMultipleEntries(): void
    {
        ...
        // 3 SRATIONALs = 24 bytes → out-of-line at offset 26 from TIFF base
        $extraData = $this->sRational(true, 1, 4)
            . $this->sRational(true, -1, 2)
            . $this->sRational(true, 5, 10);

        $payload = $this->buildPayload(true, [
            ['tag' => 0x0008, 'type' => self::TIFF_TYPE_SRATIONAL, 'count' => 3, 'value' => $this->offsetField(true, 26)],
        ], $extraData);
    ...
    public function parseHandlesOutOfLineAsciiData(): void
    {
        ...
        // "ABCDE\0" is 6 bytes → out-of-line
        $asciiString = "ABCDE\x00";
        $extraData   = $asciiString;

        $payload = $this->buildPayload(true, [
            ['tag' => 0x0011, 'type' => self::TIFF_TYPE_ASCII, 'count' => 6, 'value' => $this->offsetField(true, 26)],
        ], $extraData);
    ...
    #[Test]
    public function parseReturnsNullForIfdOffsetBeyondPayload(): void
    {
        $parser = new AppleJpegIfdParser();

        // Valid TIFF header but IFD offset points way past end
        $raw = "Apple iOS\x00" . "\x00\x01" . "MM" . pack('n', 0x002A) . pack('N', 0xFFFF);
    ...
    public function decoderProducesAppleMakerNotesFromIfdPayload(): void
    {
        ...
        // After 6 entries: sig(10)+ver(2)+TIFF(8)+count(2)+6*entry(72)+nextIfd(4)=98
        // Extra data offset from TIFF base: 98-12=86
        $payload = $this->buildPayload(true, [
            ['tag' => 0x0004, 'type' => self::TIFF_TYPE_SLONG, 'count' => 1, 'value' => $this->inlineSLong(true, 1)],
            ...
            ['tag' => 0x0011, 'type' => self::TIFF_TYPE_ASCII, 'count' => strlen($contentId), 'value' => $this->offsetField(true, 86)],
        ...
        self::assertTrue($apple->autoExposure?->aeStable);
        self::assertEqualsWithDelta(200.0, $apple->autoExposure?->aeTarget, 1e-12);
        self::assertEqualsWithDelta(180.0, $apple->autoExposure?->aeAverage, 1e-12);
        self::assertSame('HDR Image', $apple->hdr?->hdrImageType);
    }
